...
# Run MIP Baseline
sol_MIP = MIPExpansion(data, time_limit, 0, 1)
filename = "results/MIPExpanded_" * string(time_limit) * "_" * date_today
if sol_MIP != 0
    writeSolution(filename, data, sol_MIP)
end

# Run ALNS without modelRepair
sol1, params1 = ALNSExpanded(data, time_limit, false, T, alpha, gamma, frac_cluster, frac_random, thres_worst, frac_related)
filename = "results/ALNSExpanded_without_" * string(time_limit) * "_" * date_today
writeSolution(filename, data, sol1)
writeParameters(filename * "_parameters", params1) 

# Run ALNS with modelRepair
sol2, params2 = ALNSExpanded(data, time_limit, true, T, alpha, gamma, frac_cluster, frac_random, thres_worst, frac_related)
filename = "results/ALNSExpanded_with_" * string(time_limit) * "_" * date_today
