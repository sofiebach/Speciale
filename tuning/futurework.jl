...
function ALNS(data,sol,time_limit,type="extended",modelRepair=false,theta=0.005,alpha=0.99975,W=[10,9,1],gamma=0.75,destroy_frac=0.1,segment_size=10,long_term_update=0.0025)    
    it = 1
    ...
    elseif type == "extended"
        ...
        repair_functions = [bestRepair!,horizontalModelRepair!]
        destroy_functions = [VerticalRemoval,RandomRemoval,ConcurrentRemoval]
        n_d = length(destroy_functions)
        if modelRepair
            n_r = length(repair_functions)
        else
