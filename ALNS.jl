...
function ALNS_final(data,sol,time_limit,type="baseline",repairs=[true,true,true,true,true,true,true],destroys=[true,true,true,true,true,true])    
    it = 1
    best_sol = deepcopy(sol)
    temp_sol = deepcopy(sol)
    start_time = time_ns()
    ...
    if type == "baseline"
        ...
        #     end
        # end
        destroys[5:6] = [false, false]
        repairs[5:6] = [false, false]
        destroy_functions = destroy_functions[destroys]
        destroy_fracs = destroy_fracs[destroys]
        repair_functions = repair_functions[repairs]
    elseif type == "extended"
        T_start = -theta*temp_sol.exp_obj/log(0.5)
        # repair_functions = [greedyRepair!, firstRepair!,flexibilityRepair!,bestRepair!,horizontalModelRepair!,regretRepair!,modelRepair!]
        # destroy_functions = [HorizontalRemoval,VerticalRemoval,RandomRemoval,SimilarityRemoval,WorstIdleRemoval,ConcurrentRemoval]
