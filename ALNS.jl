...
function ALNS(data,sol,time_limit,type="baseline",modelRepair=false,theta=0.005,alpha=0.99975,W=[10,9,1],gamma=0.75,destroy_frac=0.1,segment_size=10,long_term_update=0.0025)    
    it = 1
...
function ALNS_final(data,sol,time_limit,type="baseline",modelRepair=false,theta=0.005,alpha=0.99975,W=[10,9,1],gamma=0.75,destroy_frac=0.1,segment_size=10,long_term_update=0.0025)    
    it = 1
    best_sol = deepcopy(sol)
    temp_sol = deepcopy(sol)
    start_time = time_ns()
...
function ALNS_final_methodinput(data,sol,time_limit,type="baseline", repairs = [true, true, true, true, true, true, true], destroys = [true, true, true, true, true, true])    
    it = 1
    best_sol = deepcopy(sol)
    temp_sol = deepcopy(sol)
    start_time = time_ns()
    ...
    if type == "baseline"
        ...
        for i in collect(StepRange(length(repair_functions), -1, 1))
            if !repairs[i]
                deleteat!(repair_functions, i)
            end
        end
        for i in collect(StepRange(length(destroy_functions), -1, 1))
            if !destroys[i]
                deleteat!(destroy_functions, i)
                deleteat!(destroy_fracs, i)
            end
        end
        n_d = length(destroy_functions)
        n_r = length(repair_functions)
