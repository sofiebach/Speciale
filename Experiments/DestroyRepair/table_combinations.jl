...
for i = 1:(length(repair_names))
    ...
    write(outFile, "\\multicolumn{1}{r|}{")
    write(outFile, r[1:end-7]*"} & ")
    write(outFile, join(total_table[:,i], " & "))
    write(outFile, "&" * string(round(sum(total_table[:,i]), sigdigits=sigdigits)))
    write(outFile, "\\\\ \n")
end
write(outFile, "& ")
write(outFile, "\\textbf{Total} & ")
write(outFile, join(round.(sum(total_table, dims=2), sigdigits=sigdigits), " & "))
close(outFile)
